            ...
            Intent.ACTION_MY_PACKAGE_REPLACED -> {
                ...
                if (version < 200) {
                    getManager(context).apply {
                        clearUserRestriction(componentName, UserManager.DISALLOW_INSTALL_UNKNOWN_SOURCES)
                        clearUserRestriction(componentName, UserManager.DISALLOW_BLUETOOTH_SHARING)
                    }
                }

            }
            else -> { Log.d(TAG, "Got an unhandled intent!") }
        }
    }
